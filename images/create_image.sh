...
# Modifications copyright (C) 2020 Carnegie Mellon University

set -eux

# Create a minimal Debian distribution in a directory.
DIR=chroot
PREINSTALL_PKGS=openssh-server,curl,tar,gcc,libc6-dev,time,strace,sudo,less,psmisc,selinux-utils,policycoreutils,checkpolicy

# If ADD_PACKAGE is not defined as an external environment variable, use our default packages
if [ -z ${ADD_PACKAGE+x} ]; then
    ADD_PACKAGE="make,git,vim,tmux,usbutils,tcpdump,gdb,gdbserver,systemtap"
fi
...
echo '/dev/root / ext4 defaults 0 1' | sudo tee -a $DIR/etc/fstab
...
echo "analysis-vm" | sudo tee $DIR/etc/hostname
...
#copy in info for systemtap
KERNELRELEASE=$(cat $KERNEL/include/config/kernel.release)
sudo mkdir -p $DIR/usr/src/linux-headers-$KERNELRELEASE
#sudo mkdir -p $DIR/lib/modules/$KERNELRELEASE/build
#sudo cp -r $KERNEL/usr/include $DIR/usr/src/linux-headers-$KERNELRELEASE
echo "kernelbuild /lib/modules/$KERNELRELEASE/build 9p defaults 0 0" | sudo tee -a $DIR/etc/fstab

#shared directory
echo "sharedworkspace /shared 9p defaults 0 0" | sudo tee -a $DIR/etc/fstab

# Build a disk image
IMAGE=${RELEASE}_freshinstall.img
...
qemu-img create -f qcow2 -b $IMAGE $RELEASE.img